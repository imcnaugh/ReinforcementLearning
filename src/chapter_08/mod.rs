...
pub fn tabular_dyna_q<S: State>(iteration_count: usize, states: Vec<S>, discount_rate: f64, size_step_parameter: f64, n: usize) {
    ...
    (0..iteration_count).for_each(|_| {
        ...
        let next_state_best_action_value = get_max_value_of_state_actions(&state_action_values, &next_state).unwrap_or(0.0);
        let new_state_action_value = current_state_action_value + (
            size_step_parameter * (reward + (discount_rate * next_state_best_action_value) - current_state_action_value)
            );
        state_action_values.insert(state_action_id.clone(), new_state_action_value);
        model.insert((state.get_id(), action), (new_state_action_value, next_state.get_id()));
        ...
        (0..n).for_each(|_| {
            ...
            let best_ns_value = get_max_value_of_state_actions(&state_action_values, &ns).unwrap_or(0.0);
...
fn get_max_value_of_state_actions<S: State>(state_action_values: &HashMap<String, f64>, state: &S) -> Option<f64> {
