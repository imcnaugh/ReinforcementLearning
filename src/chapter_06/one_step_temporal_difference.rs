...
pub fn value_function<P: Policy>(
    starting_state: BlackJackTestState,
    policy: &P,
    size_step_parameter: f64,
    discount_rate: f64,
) -> Vec<BlackJackTestState> {
    ...
    while state.is_terminal() == false {
        ...
        let error =
            reward + (discount_rate * new_state.get_state_value()) - state.get_state_value();
...
mod tests {
    ...
    fn test_value_function() {
        ...
        (10..17).for_each(|player_count| {
            policy
                .set_action_for_state(format!("{}_{}_{}", player_count, false, 10).as_str(), "hit");
            policy
                .set_action_for_state(format!("{}_{}_{}", player_count, true, 10).as_str(), "hit");
        });
        (17..=21).for_each(|player_count| {
            policy.set_action_for_state(
                format!("{}_{}_{}", player_count, false, 10).as_str(),
                "stand",
            );
            policy.set_action_for_state(
                format!("{}_{}_{}", player_count, true, 10).as_str(),
                "stand",
            );
        });
